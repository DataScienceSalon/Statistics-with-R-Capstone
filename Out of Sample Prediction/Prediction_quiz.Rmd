...
```{r Q1}
pred = exp(predict(model.AIC, ames_test))
mean(pred)
...
# Calculate RMSE
sqrt(mean(resid.AIC.train^2))
sqrt(mean(resid.full.train^2))
sqrt(mean(resid.AIC.test^2))
sqrt(mean(resid.full.test^2))
...
# Calculate proportion of observations that fall within prediction intervals
coverage.prob.full <- mean(ames_test$price > predict.BIC[,"lwr"] &
...
round(pred.test.MPM.coverage, 3)
